...
interface EnhancedTableProps {
  numSelected: number
  onRequestSort: (event: React.MouseEvent<unknown>, property: keyof Command) => void
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler = (property: keyof Command) => (event: React.MouseEvent<unknown>) => {
...
export default function TableList({ commands }: Props) {
  const rows = commands
  const [order, setOrder] = React.useState<Order>("asc")
  const [orderBy, setOrderBy] = React.useState<keyof Command>("createdAt")
  const [selected, setSelected] = React.useState<readonly string[]>([])
  const [page, setPage] = React.useState(0)
  const [dense, setDense] = React.useState(false)
  const [rowsPerPage, setRowsPerPage] = React.useState(10)

  const handleRequestSort = (event: React.MouseEvent<unknown>, property: keyof Command) => {
